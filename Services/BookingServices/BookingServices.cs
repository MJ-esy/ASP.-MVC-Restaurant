...
namespace ASP.MVC.Services.BookingServices
{
  public class BookingServices : IBookingServices
  {
    ...
    public async Task<CreateBookingDTO> CreateBooking(CreateBookingDTO newBooking)
    {
      var response = await _client.PostAsJsonAsync("Bookings/createBooking", newBooking);
      if (!response.IsSuccessStatusCode)
      {
        Console.WriteLine(response.StatusCode);
        return null;
      }
      var createdBooking = await response.Content.ReadFromJsonAsync<CreateBookingDTO>();
      if (createdBooking != null) return createdBooking;
      return null;
    }

    public async Task<UpdateBookingDTO> UpdateBooking(int id, UpdateBookingDTO updatedBooking)
    {
      ...
      var booking = await response.Content.ReadFromJsonAsync<UpdateBookingDTO>();
      return booking;
    }
